    ...
    implements IDatabaseCacheAdapter
{
    ...
    async init() {
        ...
        try {
            ...
            if (embeddingConfig.provider === EmbeddingProvider.OpenAI) {
                ...
                await client.query("SET app.use_heurist_embedding = 'false'");
                ...
                await client.query("SET app.use_heurist_embedding = 'false'");
                await client.query("SET app.use_gaianet_embedding = 'false'");
            } else if (embeddingConfig.provider === EmbeddingProvider.Heurist) {
                await client.query("SET app.use_openai_embedding = 'false'");
                await client.query("SET app.use_ollama_embedding = 'false'");
                await client.query("SET app.use_heurist_embedding = 'true'");
                await client.query("SET app.use_gaianet_embedding = 'false'");
    ...
    async getAccountByUserAddress(
        ...
        }, "getAccountByUserAddress"); // Updated context name
    }
        ...
        return this.withDatabase(async () => {
            try {
                ...
                await this.pool.query(
                  ...
                  VALUES ($1, $2, $3, $4, $5, $6, $7, $8)`, // Added userAddress and safeAddress columns
                    [
                        accountId,
                        account.name,
                        account.username || "",
    ...
    async searchKnowledge(params: {
        ...
        return this.withDatabase(async () => {
            try {
                ...
                elizaLogger.debug("Starting searchKnowledge:", {
                ...
                // First, let's check if we have any matching records at all
                const checkQuery = `
                    SELECT COUNT(*) as count
                    FROM knowledge
                    WHERE ("agentId" IS NULL AND "isShared" = true) OR "agentId" = $1
                `;

                const countResult = await this.pool.query(checkQuery, [
                    params.agentId,
                ]);
                elizaLogger.debug("Total available records:", {
                    count: countResult.rows[0].count,
                });

                // Super simple query just to get ANY results
                const sql = `
                    SELECT
                        k.*,
                ...
                elizaLogger.debug("Executing query with params:", {
                ...
                elizaLogger.debug("Query results:", {
                    rowCount: rows.length,
                    firstRow: rows[0]
                        ? {
                              hasEmbedding: !!rows[0].embedding,
                              similarity: rows[0].similarity,
                              contentPreview: rows[0].content?.text?.substring(
                                  0,
                                  100
                              ),
                          }
                ...
                elizaLogger.error("Search failed:", {
                    error:
                        error instanceof Error ? error.message : String(error),
                    query: "Simplified vector search",
                    params: {
                        agentId: params.agentId,
                        hasEmbedding: !!params.embedding,
                        searchText: params.searchText,
