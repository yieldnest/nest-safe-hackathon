    ...
    async init() {
        ...
        await this.withTransaction(async (tx) => {
            ...
            if (embeddingConfig.provider === EmbeddingProvider.OpenAI) {
                ...
                await tx.query("SET app.use_heurist_embedding = 'false'");
                ...
                await tx.query("SET app.use_openai_embedding = 'false'");
                await tx.query("SET app.use_ollama_embedding = 'false'");
                await tx.query("SET app.use_heurist_embedding = 'true'");
                await tx.query("SET app.use_gaianet_embedding = 'false'");
            ...
            elizaLogger.info("Initializing database schema...");

            try {
                await tx.exec(schema);
                elizaLogger.info("Schema initialized successfully");
            } catch (error) {
                elizaLogger.error("Error initializing schema:", error);
                throw error;
            }
        }, "init");
    }
