...
type imageUrl = { url: string; weight?: number };
...
export const generateImage = async (
            ...
            : (() => {
                  ...
                  switch (runtime.imageModelProvider) {
                      ...
                      default:
                          ...
                          return (
                              runtime.getSetting("LUMA_API_KEY") ??
    ...
    try {
        if (runtime.imageModelProvider === ModelProviderName.LUMA) {
            const response = await fetch(
                "https://api.lumalabs.ai/dream-machine/v1/generations/image",
                {
                    ...
                    body: JSON.stringify({
                        prompt: data.prompt,
                        character_ref: data.characterRef || {},
            ...
            if (!response.ok) {
                throw new Error(
                    `Luma image generation failed: ${response.statusText}`
                );
            }
            ...
            if (result.state === "completed" && result.assets?.image) {
                return {
                    success: true,
                    data: [result.assets.image],
                };
            } else if (result.failure_reason) {
                throw new Error(
                    `Luma generation failed: ${result.failure_reason}`
                );
            } else {
                throw new Error("Luma generation did not return an image URL");
            }
        } else if (runtime.imageModelProvider === ModelProviderName.HEURIST) {
            const response = await fetch(
