 ...
 * action registration, and interaction with external services like OpenAI.
 */
export class AgentRuntime implements IAgentRuntime {
    ...
    /**
     * Process an action for the agent to perform.
     * @param actionName The name of the action to process.
     * @param message The message to process the action from.
     * @param state The state of the agent.
     * @param callback The handler callback.
     * @param options The options to pass to the action handler.
     */
    async processAction(
        actionName: string,
        message: Memory,
        ...
        if (!action) {
            elizaLogger.error(`Action ${actionName} not found.`);
            return;
        }
        if (!action.handler) {
