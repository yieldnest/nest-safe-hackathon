...
export default {
    ...
    similes: [
        ...
        "GET_ARBITRUM_PRICE_HISTORY",
        "SHOW_ARBITRUM_PAIR_CANDLES",
        "CHECK_ARBITRUM_PRICE_CHART",
    ],
    ...
    description:
        "Get OHLCV (price history) data for a specific trading pair on Arbitrum blockchain",
    handler: async (
        ...
        elizaLogger.log("Starting Moralis GET_ARBITRUM_PAIR_OHLCV handler...");
        ...
        try {
            ...
            const ohlcvContext = composeContext({
                state: currentState,
                template: getPairOHLCVTemplateArbitrum,
            });
            ...
            if (!content.pairAddress) {
                throw new Error("No Arbitrum pair address provided");
            }
            ...
            const url = API_ENDPOINTS.ARBITRUM.PAIR_OHLCV(content.pairAddress);
            ...
            if (candles.length === 0) {
                if (callback) {
                    callback({
                        text: "No price history data available for this Arbitrum trading pair in the specified time range.",
                        content: {
                            candles: [],
            ...
            if (callback) {
                ...
                let summaryText =
                    `Here's the ${timeframe} price history in ${currency} for the Arbitrum trading pair ` +
        ...
        [
            {
                user: "{{user1}}",
                content: {
                    text: "Show me last 15 candles for Arbitrum pair 0xb18ea0b56e46d4f890dc729425184fd1ef25dbff",
                },
            },
            {
                user: "{{agent}}",
                content: {
                    text: "I'll retrieve the last 15 candlesticks for this Arbitrum trading pair.",
                    action: "GET_ARBITRUM_PAIR_OHLCV",
                },
            },
        ],
        [
            {
                user: "{{user1}}",
                content: {
                    text: "Get hourly candlesticks for past 2 days for Arbitrum pair 0x1f98431c8ad98523631ae4a59f267346ea31f984",
                },
            },
            {
                user: "{{agent}}",
                content: {
                    text: "I'll fetch the hourly price history for this Arbitrum trading pair.",
                    action: "GET_ARBITRUM_PAIR_OHLCV",
                },
            },
        ],
