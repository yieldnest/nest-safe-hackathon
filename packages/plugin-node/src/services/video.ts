...
export class VideoService extends Service implements IVideoService {
    ...
    private async getTranscript(
        ...
        try {
            // Validate inputs
            if (!url || !videoInfo) {
                elizaLogger.log("Missing required parameters");
                return "Unable to get transcript: Invalid video information";
            }
            // Check for manual subtitles
            if (videoInfo.subtitles?.en?.[0]?.url) {
                ...
                try {
                    ...
                    return (
                        this.parseSRT(srtContent) || "Unable to parse subtitles"
                    );
                } catch (e) {
                    elizaLogger.log(
                        "Error downloading/parsing manual subtitles:",
                        e
                    );
                }
            }
            ...
            if (videoInfo.automatic_captions?.en?.[0]?.url) {
                elizaLogger.log("Automatic captions found");
                try {
                    const captionUrl = videoInfo.automatic_captions.en[0].url;
                    const captionContent =
                        await this.downloadCaption(captionUrl);
                    return (
                        this.parseCaption(captionContent) ||
                        "Unable to parse captions"
                    );
                } catch (e) {
                    elizaLogger.log(
                        "Error downloading/parsing automatic captions:",
                        e
                    );
                }
            }
            ...
            if (videoInfo.categories?.includes("Music")) {
                ...
                return "No lyrics available for this music video.";
            }
            ...
            try {
                return (
                    (await this.transcribeAudio(url, runtime)) ||
                    "Unable to transcribe audio"
                );
            } catch (e) {
                elizaLogger.log("Error transcribing audio:", e);
                return "Unable to get transcript: Audio transcription failed";
            }
        } catch (error) {
            elizaLogger.log("Error in getTranscript:", error);
            return "Unable to get transcript: An unexpected error occurred";
        }
    }
    ...
    private async downloadAudio(
        ...
        let tempMp4File: string | null = null;
        ...
        try {
            ...
            if (tempMp4File && fs.existsSync(tempMp4File)) {
                try {
                    fs.unlinkSync(tempMp4File);
                } catch (error) {
                    elizaLogger.log("Error deleting temp MP4 file:", error);
                }
            }
        }
    }
