...
export async function validateNodeConfig(
    ...
    try {
        ...
        const modelProvider = runtime.getSetting(
            "MODEL_PROVIDER"
        ) as ModelProviderName;

        // Only include what's absolutely required
        const config = {
            // Include OpenAI or Ollama settings based on provider
            ...(modelProvider === "ollama"
                ? {
                      OLLAMA_HOST:
                          runtime.getSetting("OLLAMA_HOST") ||
                          process.env.OLLAMA_HOST ||
                          "http://localhost:11434",
                      OLLAMA_MODEL:
                          runtime.getSetting("OLLAMA_MODEL") ||
